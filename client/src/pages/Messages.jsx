        ...
        <div className='flex flex-col gap-3'>
          {dummyConnectionsData.map((user)=> (
            <div key={user._id} className='max-w-xl flex flex-warp gap-5 p-6 bg-white shadow rounded-md'>
              <img onClick={()=> navigate(`/profile/${user._id}`)} src={user.profile_picture} className='cursor-pointer rounded-full size-12 mx-auto' alt="" />
              <div className='flex-1'>
                <p className='font-medium cursor-pointer text-slate-700'>{user.full_name}</p>
